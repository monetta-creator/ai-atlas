...
// The intel desk's day grid (the contribution-calendar form, cloned from
// ScanCalendar): one cell per calendar day, completed days on a single-hue
// intensity ramp by item volume, failed days in the reserved status color,
// empty days outlined. Identity is never color-alone: every cell carries an
// aria-label, the hover tooltip restates everything, and the legend labels
// each state. Completed cells ARE the archive: clicking one downloads that
// day's intel-items JSON.

export interface IntelCalDay {
...
function cellStyle(d: IntelCalDay | null): React.CSSProperties {
  ...
  if (d.status === 'completed') {
    return { ...base, background: 'var(--supports)', opacity: intensity(d.feed + d.search + d.filings) };
  }
...
function labelOf(d: IntelCalDay): string {
  if (!d.status) return isWeekend(d.day) ? `${d.day}: scheduled off (weekend)` : `${d.day}: no run`;
  const items = d.feed + d.search + d.filings;
  return `${d.day}: ${d.status}, ${items} items (${d.feed} feed, ${d.search} search, ${d.filings} filings), ${d.hydrated} hydrated, ${d.enriched} enriched, ${d.facts} fact${d.facts === 1 ? '' : 's'}${
    typeof d.cost === 'number' ? `, $${d.cost.toFixed(2)}` : ''
  }${d.downloadHref ? '. Click to download the day as JSON.' : ''}`;
}

export default function IntelCalendar({ days }: { days: IntelCalDay[] }) {
  ...
  const padded: (IntelCalDay | null)[] = [...Array<null>(firstDow).fill(null), ...days];
  const weeks: (IntelCalDay | null)[][] = [];
  ...
  const show = (d: IntelCalDay) => (e: React.MouseEvent) =>
    ...
    <div>
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs" style={{ color: 'var(--faint-ink)', marginTop: 8 }}>
        <span className="flex items-center gap-1">
          <span style={{ ...cellStyle({ day: '', status: null, feed: 0, search: 0, filings: 0, hydrated: 0, enriched: 0, facts: 0, cost: null, downloadHref: null }), width: 11, height: 11 }} />
          no run
        </span>
        <span className="flex items-center gap-1">
