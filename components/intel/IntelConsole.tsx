...
// Client driver for a manual intel run/resume, mirroring ScanConsole: each
// tick is one short server action (at most one bounded work unit), looped
// until the day's run completes. The cron pair is the scheduled driver; this
...
export default function IntelConsole() {
  ...
  async function run() {
    ...
    try {
      say('▶ Intel desk…');
      const { runId, day } = await startOrResumeIntelAction();
      say(`  run ${runId.slice(0, 8)} · day ${day}`);
      let lastLine = '';
      // Safety cap far above any real day (feeds + per-company search/filings
      // waves + hydrate/enrich).
      for (let tick = 0; tick < 400; tick++) {
        const p = await intelTickAction(runId);
        ...
        const c = p.counters;
        const line = `  ${p.step} · feeds ${c.feedItems} · search ${c.searchItems} · filings ${c.filingItems} · hydrated ${c.hydrated} · enriched ${c.enriched} · skipped ${c.skipped} · facts ${c.facts} · metrics ${c.metrics}`;
        if (line !== lastLine) {
          say(line);
          lastLine = line;
        }
        ...
        if (p.done) {
          say('✓ Intel run complete. The datasets serve this day as the latest.');
          return;
        }
      }
    ...
    >
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-xs" style={{ color: 'var(--faint-ink)' }}>
          feeds → search → filings → hydrate → enrich, checkpointed per unit
        </span>
      </div>

      <p className="text-xs" style={{ color: 'var(--faint-ink)', marginTop: 10 }}>
        Runs (or resumes) today&apos;s intel sweep: free press feeds, rotating per-company web
        search, EDGAR filings, full-text hydration, and model enrichment (facts + tags) under the
        daily budget cap. The two daily crons normally do all of this; this button finishes a day
        they could not.
      </p>
