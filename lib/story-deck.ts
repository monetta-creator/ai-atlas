...
export async function buildStoryDeckData(): Promise<CostDeck> {
  ...
  const slides: DeckSlide[] = [
    ...
    {
      kind: 'table',
      kicker: 'Act 2 · the architecture scales, components swap',
      title: 'Each notch of the dial, priced',
      heads: ['Scale', 'Documents / day', 'Est. monthly cost', 'What swaps in', 'Cost in market terms'],
      rows: [
        ['1x (today)', '~300', `~$${runningUsd}`, 'nothing: free tiers, pennies of model spend', 'about 3 percent of one entry-level CI subscription'],
        ['10x', '~3,000', '~$400', 'a paid search tier; same engines, same database', 'about one 10-user enterprise news license'],
        ['100x', '~30,000', '~$3,600', 'firehose feeds in place of polling; object storage', 'about one enterprise media-monitoring contract'],
        ['1000x', '~300,000', '~$30,000 to $36,000', 'dedicated pipeline infrastructure', 'about a 10-seat market-intelligence deployment, reading 100M documents a year'],
      ],
      note: 'The point of this table is the fourth column: at every row the engines, the gates, the ontology, and the export contracts are UNCHANGED. Only the plumbing under them swaps. Assumptions: measured $0.004 enrichment per document on open-weight models at today’s prices (token prices have fallen roughly 10x per year, so costs are a ceiling); collection stays model-free; ~12KB retained text per document. Market bands from the 2026-08-30 comps research.',
      takeaway: 'The dollars are a footnote. The argument is that scale is a component swap, not a redesign: the architecture already absorbs every row.',
    },
    {
      kind: 'table',
      ...
      rows: [
        ['10x', 'everything: engines, budgets, gates, ontology, exports', 'none: this notch is a pricing tier'],
