  ...
  const slides: DeckSlide[] = [
    {
      ...
      subtitle:
        'The Atlas reads the outside world through one subsystem: continuous ingestion of news, filings, and regulatory data, structured on arrival. That piece is now built out and proven. This is what scaling it costs, and what every surface above it gets when the dial turns.',
    ...
    { kind: 'divider', kicker: 'Act 2', title: 'The road to 1000x', subtitle: 'The question is not whether it can be done. It is what turning the dial costs, one order of magnitude at a time.' },
    {
      ...
      rows: [
        ...
        ['1000x', '~300,000', 'under $36,000', 'dedicated inference and pipeline infrastructure', 'about ten seats of commercial tooling, reading 100M documents a year'],
      ],
    ...
    {
      kind: 'stat-grid',
      kicker: 'Act 2 · what the app gets',
      title: 'When the intake scales, the whole Atlas compounds',
      stats: [
        { n: 'Signal board', l: 'from a handful of drafts a day to a ranked stream', sub: 'the human gate stays; selection gets sharper' },
        { n: 'Argument map', l: 'claims accrue evidence faster; one-sided flags trip sooner', sub: 'confidence still moves only by hand, on more signal' },
        { n: 'Ask', l: 'the same grounded, cited answers over a corpus 1000x deeper', sub: 'retrieval and verification already in place' },
        { n: 'Company intel', l: 'dossiers, fact timelines, and metrics fill in whole niches', sub: 'from a tracked registry toward full coverage' },
        { n: 'Theses and reports', l: 'gap scans and briefs grounded in more evidence', sub: 'the estimative layer gets more to weigh' },
        { n: 'The firewall feed', l: 'same schemas, same contracts, more rows', sub: 'downstream ingestion scales with zero changes' },
      ],
      note: 'Ingestion without retrieval is a landfill, and this is the point: every surface above the intake already exists, already cites the corpus, and grows richer with every document the intake adds. Nothing above it changes shape when it scales.',
      takeaway: 'This build-out is one piece of the Atlas. Scaling it upgrades every surface above it, with zero redesign.',
    },
    {
      kind: 'divider',
      kicker: 'The answer',
      title: '1000x is arithmetic.',
      subtitle: 'The ingestion layer is one piece of the Atlas, built out and proven, with the scale dial priced per notch at worst-case retail. Turning it upgrades everything above it: the ontology keeps the reading navigable, the retrieval layers keep it answerable, and the gates keep it trustworthy at any volume.',
    },
  ];
