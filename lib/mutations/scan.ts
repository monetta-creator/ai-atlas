...
// TODAY's already-completed run at step 'feeds' so advanceScanRun re-pulls
...
// for claimScanRun's own resume path, not this one). The caller still calls
// claimScanRun right after to take the real 5-minute lease; this just flips
// the row back into a resumable state.
export async function reopenScanRunForSweep(runId: string): Promise<boolean> {
  const row = await one<{ id: string }>(
    `update scan_runs
