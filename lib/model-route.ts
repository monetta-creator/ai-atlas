...
// Shared model dispatcher for every cheap-model-routed feature across the
// app's engines (research's triage/queue-agent/analysis; the tooling
// monitor's triage/enrich/score): the scan's dual-provider pattern
// (lib/scan/enrich.ts) applied once instead of once per subsystem. `model` is
// either null/empty (the Haiku fallback), an Anthropic id (any `claude-*` id,
// or an entry flagged `anthropic: true` in SCAN_ENRICH_MODELS) routed through
...
// own system/user/schema exactly as before this dispatcher existed; only the
// model selection and provider branch move here.
//
...
// chunk (the calling engine resumes on the next invocation), matching the
// discipline lib/research/analysis.ts and lib/research/queue-agent.ts
// already used before this dispatcher existed.
//
// Never sets pipelineRunId — none of these calls are pipeline_runs rows
...
export async function routedStructured<T>(opts: {
  model: string | null;
  system: string;
  user: string;
  toolName: string;
  ...
  // The dispatcher is shared across many different schemas, so it can't hand-write
...
// signals.drafted_by, tooling_products.enriched_by/agent_model): the picked
// model, or the Haiku fallback when none was picked. Never null, so a
// stamped column always names a real model.
