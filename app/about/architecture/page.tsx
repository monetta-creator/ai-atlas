...
const SECTIONS = [
  ...
  },
  {
    ...
    body: 'The server decides what is public. The readers in lib/data/ strip the personal fields (confidences, rationales, source priors, review notes, the scout’s verdicts) before anything leaves the server for a guest; guest mode IS the share view. The authorization boundary is isAdmin() in lib/auth.ts, a signed HMAC cookie; a second signed cookie, unlocked by an access key (one shared team key today), grants the portal tier: Ask, the key-gated datasets (retained article text, machine-extracted records including unreviewed items, agent scores, dossier fields), the retained-text reader beside an answer, the tooling reports and their drafts, and Scout’s research tools and document upload. proxy.ts is open by default: every page renders sessionless and gates itself, so a visitor gets the guest view instead of a login wall, and the proxy fences only API routes outside a short allow-list. RLS is deny-by-default underneath.',
  },
  {
    id: 'human-gate',
